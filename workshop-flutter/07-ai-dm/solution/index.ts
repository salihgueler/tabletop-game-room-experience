/**
 * Backend — aws-blocks/index.ts  (MODULE 07 — AI Dungeon Master: canned → Agent)
 *
 * Change from module 06: the DM is now a real `Agent`. `narrate()` asks the model
 * to describe each action's outcome, and `nextScene()` asks it to set the scene
 * and emit 3–4 CONTEXTUAL action choices (instead of a fixed class menu) — while
 * streaming its reasoning tokens live to the `thinking` Realtime channel you wired
 * in module 06. Both keep the canned logic as a fallback, so the game still runs
 * fully offline when no model is available.
 *
 * The agent is `inferenceOnly` (stateless one-shot) — no conversation persistence,
 * so many games run concurrently and isolated. Locally it uses Ollama if present,
 * otherwise the canned provider; deployed it uses Bedrock.
 *
 * Still mocked (module 08 replaces this):
 *   - companionDecide ....... random action + canned line → per-class Agent
 *
