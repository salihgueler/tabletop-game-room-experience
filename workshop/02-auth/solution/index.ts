/**
 * Backend — aws-blocks/index.ts  (MODULE 02 — Auth: fakeAuth → AuthBasic)
 *
 * Change from the starter: the in-memory `fakeAuthApi` / `requireAuth()` are gone.
 * Auth is now a real `AuthBasic` Block — username/password accounts, hashed
 * passwords, and an HttpOnly session cookie. `auth.requireAuth(context)` reads
 * that cookie off the per-request `context` and throws 401 when there's no
 * session, so every method that used to call the fake `requireAuth()` now awaits
 * the real one.
 *
...
// ═══════════════════════════════════════════════════════════════════════════════
// MOCKS — the remaining faked concerns. Auth is now REAL (AuthBasic, above).
// Later modules delete each of these and wire in a real Block.
// ═══════════════════════════════════════════════════════════════════════════════

// ─── MOCK: persistence (Modules 03–04 → DistributedTable) ───────────────────────
