/**
 * Backend — aws-blocks/index.ts  (MODULE 04 — Guild Hall lobby: Map → DistributedTable + GSI)
 *
 * Change from module 03: the in-memory `gameStore` Map is gone. Lobby rows now
 * live in a real `DistributedTable`. The twist: a full-table `scan()` is the wrong
 * tool for a listing (unindexed, slower as the table grows), so to list every
 * public game we give all rows the SAME partition key (`listKey: "all"`) and add a
 * secondary index (`byCreated`) on it — then a single query returns them all,
 * newest-first. This "constant PK + GSI" pattern is the canonical way to list a
 * collection in Blocks.
 *
 * Still mocked (later modules replace these):
 *   - state / chat ........... in-memory Maps   (Module 05 → DistributedTable)
...
// ═══════════════════════════════════════════════════════════════════════════════
// MOCKS — the remaining faked concerns. Auth, characters, and games are now REAL.
// Later modules delete each of these and wire in a real Block.
// ═══════════════════════════════════════════════════════════════════════════════

// ─── MOCK: persistence (Module 05 → DistributedTable) ───────────────────────────
// Per-game state and chat still live in plain Maps: everything resets when the
// dev server restarts. Their access patterns mirror DistributedTable.
...
// Character and Game types now come from their table schemas (single source of
// truth). Note: Game now carries `listKey` (the constant partition key).
