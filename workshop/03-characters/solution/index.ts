/**
 * Backend — aws-blocks/index.ts  (MODULE 03 — Characters: Map → DistributedTable)
 *
 * Change from module 02: the in-memory `characterStore` Map is gone. A player's
 * hero now lives in a real `DistributedTable` (DynamoDB when deployed, a file
 * mock in `.bb-data/` locally) — so it survives a server restart. The table is
 * schema-validated with Zod and keyed by `userId`.
 *
 * Still mocked (later modules replace these):
 *   - games / state / chat .. in-memory Maps   (Modules 04–05 → DistributedTable)
...
// ═══════════════════════════════════════════════════════════════════════════════
// MOCKS — the remaining faked concerns. Auth + characters are now REAL (above).
// Later modules delete each of these and wire in a real Block.
// ═══════════════════════════════════════════════════════════════════════════════

// ─── MOCK: persistence (Modules 04–05 → DistributedTable) ───────────────────────
// Games, per-game state, and chat still live in plain Maps: everything resets
// when the dev server restarts. Their access patterns mirror DistributedTable.
...
// Character type now comes from the table's Zod schema (single source of truth).
