/**
 * Backend — aws-blocks/index.ts  (MODULE 08 — AI companions: canned → Agent × party)
 *
 * Change from module 07: each AI party seat is now its own `Agent` with a distinct
 * class persona (Paladin / Sorcerer / Rogue / Ranger). On a companion's turn its
 * agent reasons, picks ONE action from the current options, and speaks a short
 * in-character line — reasoning streamed live to the `thinking` channel. Together
 * with the DM that's a genuine multi-agent session: one DM agent + up to three
 * companion agents, each reasoning independently and isolated per game.
 *
 * This is the LAST mock removed. Every backend concern — auth, data, realtime, and
 * AI — is now a real AWS Building Block. Module 09 deploys it.
 *
...
// ═══════════════════════════════════════════════════════════════════════════════
// MOCKS — auth, data, and realtime are now ALL REAL. Only AI remains faked
// (modules 07–08).
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
// AI Dungeon Master (Agent) + canned fallback.
//
// The DM agent is inference-only (no conversation persistence). Model resolution:
// Bedrock when deployed, Ollama locally if running, canned provider otherwise.
//
// ⚠️ Model pinning: presets like BedrockModels.BALANCED are the documented default,
// but Bedrock periodically retires the underlying model — a retired id fails the
// agent's health check and silently falls back to canned (degraded, not broken).
// If deployed narration goes generic, check the Lambda logs and consider pinning
// an explicit current inference-profile id instead of the preset.
// ═══════════════════════════════════════════════════════════════════════════════
...
// ─── Canned fallback (deterministic, offline) ────────────────────────────────
function categorize(
  action: string,
...
// Ask the AI DM to narrate an outcome; fall back to canned text if the model
// errors or is unavailable. inferenceOnly → stream() then complete() for the text.
...
// Each non-human seat is driven by its own agent. This is what makes a session
// genuinely multi-agent: the DM plus up to three distinct companion agents, each
// reasoning independently. All inferenceOnly (stateless one-shot), like the DM.
...
// ═══════════════════════════════════════════════════════════════════════════════
// TURN ENGINE  — authoritative server logic (no block; unchanged since module 05)
// ═══════════════════════════════════════════════════════════════════════════════
const uid = () =>
...
// Persist the new state (bumping version) and broadcast a `state` bump so every
// subscribed client refetches getState. Returns the SAVED object so callers can
// hand it straight back to the frontend.
async function saveAndBroadcast(state: GameState) {
  const next = { ...state, version: state.version + 1 };
  await gameStates.put(next);
  await rt.publish("state", next.gameId, {
    gameId: next.gameId,
...
async function transcribe(
  ...
  for (let i = 0; i < withColor.length; i++) {
    ...
    const msg: ChatMsg = {
      gameId: state.gameId,
      ts: base + i, // unique + ordered: nextTs reserved the whole range
      who: e.who,
      color: e.color,
      text: e.text,
      kind: e.kind,
...
export const api = new ApiNamespace(scope, "api", (context) => ({
  ...
  // --- Realtime channels ---
  // Return a channel handle the frontend subscribes to. The server publishes to
  // the matching namespace (via rt.publish in saveAndBroadcast / transcribe).
